...
<div class="min-h-screen bg-gray-50">
    ...
    <div class="bg-white border-b sticky top-16 z-40">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex gap-8">
            ...
            <?php echo $tab == "dashboard" ? "border-blue-600 text-blue-600" : "border-transparent text-gray-600 hover:text-gray-900"; ?>">Dashboard</a>
            <a href="?tab=cart" class="py-4 font-medium border-b-2
            <?php echo $tab == "cart" ? "border-blue-600 text-blue-600" : "border-transparent text-gray-600 hover:text-gray-900"; ?>">Cart</a>
            <a href="?tab=messages" class="py-4 font-medium border-b-2
            <?php echo $tab == "messages" ? "border-blue-600 text-blue-600" : "border-transparent text-gray-600 hover:text-gray-900"; ?>">Messages</a>
            <a href="?tab=purchase-history" class="py-4 font-medium border-b-2
            <?php echo $tab == "purchase-history" ? "border-blue-600 text-blue-600" : "border-transparent text-gray-600 hover:text-gray-900"; ?>">Purchase History</a>
        </div>
    </div>
    ...
    <?php elseif ($tab == "cart"): ?>
            ...
            <?php if (empty($cartItems)): ?>
                <div class="bg-white rounded-2xl border border-slate-100 p-16 text-center shadow-sm">
                    ...
                    <p class="text-slate-500 mb-8 max-w-sm mx-auto">
                        Explore our wide range of skills and start your learning journey today
                    </p>
                    <a href="search-products.php" 
                       class="inline-flex items-center px-6 py-3 bg-blue-600 text-white font-bold
                              rounded-xl hover:bg-blue-700 transition-all shadow-lg active:scale-95">
                        Browse skills
                    </a>
                </div>
            <?php else: ?>
                <div class="flex flex-col lg:flex-row gap-8">
                    <div class="flex-1 space-y-4">
                        ...
                        ?>
                            <!-- Cart Item -->
                            <div onclick='openProductModal(<?= json_encode($productData) ?>);' 
                        ...
                        <div class="bg-white rounded-2xl border border-slate-100 p-6 shadow-xl sticky top-24">
                            ...
                            <div class="space-y-4 pb-6 border-b border-slate-100">
                                <div class="flex justify-between text-slate-600">
                                    <span>Subtotal</span>
                                    <span class="font-bold text-gray-900">Rs. <span id="subtotal"><?= number_format($subtotal, 2) ?></span></span>
                                </div>
                                <div class="flex justify-between text-slate-600">
                                    <div class="flex items-center gap-1.5">
                                        <span>Course Delivery Fee</span>
                                        <div class="group relative">
                                            ...
                                            <div class="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 w-48 p-2 bg-slate-900
                                                        text-white text-[10px] rounded shadow-xl opacity-0 invisible group-hover:opacity-100
                            ...
                            <div class="pt-6">
                                ...
                                <button onclick="checkout();" class="block w-full py-4 text-center bg-gradient-to-r from-blue-600 to-indigo-600
                                               hover:from-blue-700 hover:to-indigo-700 text-white font-bold rounded-xl shadow-lg hover:shadow-xl
                                               transition-all active:scale-95 mb-4">
                                    Proceed to Checkout
                                </button>
        ...
        <div id="toast" class="fixed top-5 right-5 z-50 transform translate-y-[-100px] transition-transform duration-100 pointer-events-none">
        ...
        <div class="fixed inset-0 z-[100] hidden" id="productModal">
            <!-- Overlay -->
            <div id="modalOverlay" 
                 class="absolute inset-0 bg-slate-900/70 backdrop-blur-sm opacity-0 transition-opacity duration-300" 
                 onclick="closeProductModal();"></div>

            <!-- Modal Content -->
            <div id="modalContent" 
                 class="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-11/12 max-w-4xl bg-white rounded-3xl shadow-2xl overflow-hidden flex flex-col md:flex-row transform scale-95 opacity-0 transition-all duration-300">
                ...
                <div class="md:w-1/2 relative bg-slate-100 aspect-video md:aspect-auto">
                    ...
                    <span id="modalLevel" 
                          class="absolute top-4 left-4 text-xs font-bold px-3 py-1 rounded-md bg-blue-600 text-white uppercase tracking-wider shadow-md">
                        Beginner
                    </span>
                </div>
                ...
                <div class="p-6 flex flex-col">
                    <button onclick="closeProductModal();" 
                            class="absolute top-4 right-4 text-gray-500 hover:text-gray-700 bg-gray-200 rounded-full w-8 h-8 flex items-center justify-center transition">
                        ✕
                    </button>
            ...
            <script>
                ...
                async function removeItem(cartItemId, e) {
                    ...
                    try {
                        ...
                        if (data.success) {
                            itemEl.remove();
                            document.getElementById('subtotal').innerText = parseFloat(data.subtotal).toLocaleString(undefined,{minimumFractionDigits: 2});
                            document.getElementById('delivery').innerText = parseFloat(data.delivery).toLocaleString(undefined,{minimumFractionDigits: 2});
                            document.getElementById('total').innerText = parseFloat(data.total).toLocaleString(undefined,{minimumFractionDigits: 2});
                            ...
                            if (parseFloat(data.itemCount) === 0) {
...
<script>
  ...
  async function loadMessages(){
    ...
    msgs.forEach(m => {
      ...
      const color = m.side === 'right' 
        ? 'bg-gray-900 text-white rounded-tr-none' 
        : 'bg-white border border-gray-100 text-gray-800 rounded-tl-none';

      let seenHtml = '';
      if(m.side === 'right'){
        seenHtml = m.status === 'seen' 
          ? '<span class="ml-2 text-blue-400 font-bold">✓✓</span>' 
          : '<span class="ml-2 text-blue-400 font-bold">✓</span>';
      }

      html += `
        <div class="flex ${side}">
          <div class="${color} px-5 py-3 rounded-2xl max-w-[85%] shadow-sm">
    ...
    <main class="max-w-7xl mx-auto px-4 lg:px-8 py-10">
        <div class="mb-8">
            ...
            <p class="text-gray-500 mt-2">
                View your past orders and share your feedback on the skills you've learned.
            </p>
        </div>
                ...
                <?php while($invoice = $invoiceQ->fetch_assoc()): ?>
                    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                        ...
                        <div class="px-6 py-4 bg-gray-50/50 border-b border-gray-100 flex flex-wrap justify-between items-center gap-4">
                            ...
                            <div>
                                  <a href="invoice.php?id=<?= $invoice["order_order_id"] ?>" 
                                     class="text-xs font-bold px-3 py-1.5 rounded-lg bg-white border border-gray-200 text-gray-600 hover:bg-gray-50 transition-colors shadow-sm">
                                    View Full Invoice
                                 </a>
                                 </div>
                        ...
                        <div class="divide-y divide-gray-100">
                            <?php
                                ...
                                $itemsQ = Database::search(
                                    "SELECT 
                                        ...
                                        p.`title`, 
                                        p.`image_url`, 
                                        u.`fname` AS `seller_first`, 
                                        u.`lname` AS `seller_last`, 
                                        (SELECT `id` 
                                         FROM `feedback` 
                                         WHERE `user_id` = ? 
                            ...
                            <a href="product-view.php?id=<?= $item["product_id"] ?>" 
                               class="p-6 flex flex-col md:flex-row gap-6 items-start md:items-center hover:bg-blue-100 transition-colors">
                                ...
                                <div class="flex-grow">
                                    ...
                                    <p class="text-sm text-gray-500 mt-1">
                                        ...
                                        <span class="font-medium text-gray-700">
                                            <?= $item["seller_first"] . " " . $item["seller_last"]; ?>
                                        </span>
                                    </p>
                                    <p class="text-sm font-bold text-blue-600 mt-2">
                                        Rs. <?= number_format($item["price"] ?? $invoice["total"], 2); ?>
                                    </p>
                                </div>
                                    ...
                                    <?php if (!empty($item['feedback_id'])): ?>
                                        <span class="inline-flex items-center gap-1 text-green-600 font-bold text-sm bg-green-50 px-4 py-2 rounded-xl">
                                            <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 20 20">
                                                <path d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 
                                                         7.707 9.293a1 1 0 00-1.414 1.414l2 2 4-4z"></path>
                                            </svg>
                                            Feedback Submitted
                                        </span>
                                    <?php else: ?>
                                        <button 
                                            onclick="openFeedbackModal(<?= $item['product_id'] ?>, '<?= addslashes($item['title']) ?>', event)" 
        ...
        <?php else: ?>
            <div class="bg-white rounded-3xl p-12 text-center border-2 border-dashed border-gray-100">
                <div class="w-20 h-20 bg-blue-50 text-blue-600 rounded-2xl flex items-center justify-center mx-auto mb-6 text-4xl">🛍️</div>
                <h3 class="text-xl font-bold text-gray-900">No purchases yet</h3>
                <p class="text-gray-500 mt-2 max-w-sm mx-auto">Explore our wide range of skills and start your learning journey today</p>
                <a href="home.php" class="inline-block mt-8 px-8 py-3 bg-blue-600 text-white font-bold rounded-xl hover:bg-blue-700 transition-all shadow-lg shadow-blue-200">Start Shopping</a>
            </div>
        <?php endif; ?>
    </main>
       ...
       <div class="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full max-w-md p-4">
           <div class="bg-white rounded-3xl shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-300">
              <div class="p-8">
                ...
                <p id="modal-product-title" class="text-blue-600 font-medium text-sm mb-6"><?= $item["title"]; ?></p>
                ...
                <form id="feedback-form" onsubmit="submitFeedback(event);">
                    ...
                    <div class="mb-8">
                       <label for="modal-message" class="block text-sm font-bold text-gray-700 mb-2">Your Experiance</label>
                       <textarea id="modal-message" rows="4" required placeholder="Share your thoughts about this skill..." 
                       class="w-full px-4 py-3 rounded-2xl bg:gray-50 border-trasparent focus:bg-white focus:border-blue-500
                    ...
                    <div class="flex gap-4">
                       ...
                       <button type="submit" class="flex-1 px-6 py-3 bg-blue-600 text-white font-bold rounded-2xl
                       hover:bg-blue-700 transition-all shadow-lg shadow-blue-200">Submit</button>
                    </div>
