...
$usersResult = Database::search(
    "SELECT COUNT(DISTINCT `id` ) AS `total_users` 
    FROM `user`;"
);
$productsCountResult = Database::search(
    "SELECT COUNT( `id`) AS `total_products` 
    FROM `product`;
    "
);
$feedbackStatusResult = Database::search(
    "SELECT AVG(`rating`) AS `avg_rating` 
    FROM `feedback`;"
);
$ordersStatusResult = Database::search(
...
$avgRating = ($feedbackStatusResult && $row = $feedbackStatusResult->fetch_assoc()) ? round(($row['avg_rating'] / 5) * 100,  0) : 0;
$totalOrders = ($ordersStatusResult && $row = $ordersStatusResult->fetch_assoc()) ? $row['total_orders'] : 0;
$totalRevenue = ($ordersStatusResult && $row = $ordersStatusResult->fetch_assoc()) ? $row['total_revenue'] : 0;
        ...
        <div class="grid md:grid-cols-2 gap-8 items-center">
            ...
            <div>
                ...
                <p class="text-blue-100 text-lg mb-6">
                    Connect with experet skill sellers and buyers in our thrivingcommunity
                </p>
                <!-- search Bar  -->
                <form method="GET" action="search-products.php" class="mb-6 bg-white rounded-lg p-3 flex gap-2">
                    <input type="text" name="q" placeholder="What skill are your looking for?"
                        class="flex-1 bg-transparent text-gray-900 outline-none text-sm" />
                    <button
                        class="bg-blue-600 text-white px-6 py-2 rounded-lg hover:bg-blue-700 font-medium text-sm transition-colors">Search</button>
                </form>
...
<!-- Featured Carousel -->
<section id="browse" class="py-16 md:py-24 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
    <div class="mb-12">
    ...
    <div class="relative group">
        <div class="overflow-hidden rounded-xl">
            <div id="carousel" class="flex transition-transform duration-500 ease-out" style="transform:translateX(0);">
                        ...
                        <div class="carousel-item flex-shrink-0 w-full md:w-1/3 px-3 pb-4">
                            <div class="bg-white rounded-xl overflow-hidden shadow-lg hover:shadow-2xl transition-all h-full">
                                ...
                                <div class="p-5 text-left">
                                    <h3 class="font-bold text-lg mb-2 text-left">
                ...
                <?php else: ?>
                    <!-- No Products -->
                    <div class="carousel-item flex-shrink-0 w-full md:w-1/3 px-3 pb-4">
                        <div class="bg-white rounded-xl overflow-hidden shadow-lg p-8 text-center h-full flex flex-col justify-center">
                            <p class="text-gray-600 text-sm mb-4">No products available yet. Check back soon!</p>
                        </div>
                    </div>
                <?php endif ?>
            </div>
        ...
        <!-- Navigation Buttons -->
        <button onclick="slideCarousel(-1);" class="absolute left-0 top-1/2 -translate-y-1/2 bg-blue-600 text-white p-3 rounded-full hover:bg-blue-700 transition-all hover:scale-110 z-10 opacity-0 group-hover:opacity-100">👈</button>
        <button onclick="slideCarousel(1);" class="absolute right-0 top-1/2 -translate-y-1/2 bg-blue-600 text-white p-3 rounded-full hover:bg-blue-700 transition-all hover:scale-110 z-10 opacity-0 group-hover:opacity-100">👉</button>
    </div>
...
<section id="categories" class="bg-gray-50 py-16 md:py-24">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
            ...
            <a href="#"
                class="group bg-white p-6 rounded-lg text-center shadow hover: shadow-lg transition-all cursor-pointer hover:scale-105 hover:border-2 hover:border-blue-600">
        ...
        <?php if (count($testimonials) > 0): ?>
            <?php foreach ($testimonials as $idx => $testimonial): ?>
                <div class="bg-white rounded-lg shadow-lg p-8 hover: shadow-2xl transition-shadow hover:scale-105 duration-300">
                    ...
                    <p class="text-gray-600"><?php echo $testimonial["messege"]; ?></p>
                </div>
            <?php endforeach; ?>
        <?php else: ?>

            <div class="bg-white rounded-lg shadow-lg p-8 hover: shadow-2xl transition-shadow hover:scale-105 duration-300">
                <p class="text-gray-500">No testimonial available yet. Be the first to review!</p>
            </div>
...
<section class="py-16 md:py-24 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-6">
        ...
        <div class="text-center group">
            <div
                class="bg-gradient-to-br from-blue-500 to-indigo-600 rounded-full w-16 h-16 flex items-center justify-center text-2xl text-white mx-auto mb-4 group-hover:scale-110 transition-transform">
                1</div>
        ...
        <div class="text-center group">
            <div
                class="bg-gradient-to-br from-blue-500 to-indigo-600 rounded-full w-16 h-16 flex items-center justify-center text-2xl text-white mx-auto mb-4 group-hover:scale-110 transition-transform">
                2</div>
            <h3 class="font-bold text-lg text-gray-900 mb-2">Browser or List</h3>
            <p class="text-gray-600 text-sm">Explor skills or create your own content</p>
        </div>
        <div class="text-center group">
            <div
                class="bg-gradient-to-br from-blue-500 to-indigo-600 rounded-full w-16 h-16 flex items-center justify-center text-2xl text-white mx-auto mb-4 group-hover:scale-110 transition-transform">
            ...
            <p class="text-gray-600 text-sm">Interract with instructors and commiunity</p>
        </div>
        <div class="text-center group">
            <div
                class="bg-gradient-to-br from-blue-500 to-indigo-600 rounded-full w-16 h-16 flex items-center justify-center text-2xl text-white mx-auto mb-4 group-hover:scale-110 transition-transform">
...
<script>
    var currentSlide = 0;
    var itemsPerView, maxSlides, totalItems;
    ...
    function updateViewSettings() {
        ...
        maxSlides = Math.ceil(totalItems / itemsPerView) - 1;
        ...
        if (dotsContainer) {
            ...
            for (var i = 0; i <= maxSlides; i++) {
                ...
                dot.className = "carousel-dot w-3 h-3 rounded-full transition-all " +
    ...
    function updateCarousel() {
        ...
        // Move by full "view width" (100% per group of items)
