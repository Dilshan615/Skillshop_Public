...
$q = Database::search(
    "SELECT p.`id`, p.`title`, p.`image_url`, p.`level`, p.`price`, p.`created_at`,
        u.`fname` AS `seller_name`,
        AVG(COALESCE(f.`rating`,0)) AS `avg_rating`, COUNT(f.`id`) AS `review_count`,
        w.`id` AS `watch_id`
    FROM `watchlist` w
...
<div class="min-h-screen relative overflow-hidden">
  ...
  <div class="flex flex-col md:flex-row md:items-end md:justify-between gap-6 mb-10">
    <div>
      ...
      <h1 class="text-3xl md:text-4xl font-extrabold text-gray-900 tracking-tight">My Watchlist</h1>
      <p id="itemsCount" class="text-gray-500 mt-2"><?= count($items) ?> Skill<?= count($items) != 1 ? "s" : "" ?> saved for later</p>
    </div>
    <a href="search-products.php" class="inline-flex items-center gap-2 px-6 py-3 rounded-xl font-semibold bg-white border-2
     border-gray-200 text-gray-700 hover:border-blue-400 hover:text-blue-600 shadow-sm hover:shadow-md transition-all shrink-0">
  ...
  <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6" id="watchlist-grid">
    <?php foreach($items as $p): 
        ...
        $rating = $p["review_count"] > 0 ? round($p["avg_rating"], 1) : "New";    
    ?>
     <article class="group relative bg-white/80 backdrop-blur rounded-2xl border
     border-gray-100 shadow-lg hover:shadow-2xl overflow-hidden transition-all
     duration-100 hover:translate-y-1" data-product-id="<?= $p["id"]; ?>">
      <a href="product-view.php?id=<?= $p["id"]; ?>" class="block" >
        <div class="relative h-44 bg-gradient-to-br from-slate-100 to-slate-50 overflow-hidden">
           ...
           <span class="absolute top-3 left-3 px-3 py-1 bg-white/90 backdrop-blur text-xs font-bold text-blue-700 rounded-lg shadow-sm"><?= $p["level"]; ?></span>
        </div>
        <div class="p-5">
            <p class="text-xs font-semibold text-blue-600 mb-1"><?= ($p["seller_name"] ?? "Instructor") ?></p>
            <h3 class="font-bold text-gray-900 text-base leading-snug line-clamp-2 group-hover:text-blue-600 transition-colors"><?= $p["title"]; ?></h3>
            <div class="flex items-center justify-between mt-4">
                <span class="flex items-center gap-1 text-sm">
                    <span class="text-amber-400">★</span><?= $rating; ?>
                </span>
                <span class="text-lg font-bold bg-gradient-to-r from-blue-600 to-indigo-600 bg-clip-text text-transparent">Rs. <?= number_format($p["price"],2); ?></span>
            </div>
        </div>
      </a>
      <button type="button" class="wl-heart absolute top-3 right-3 z-10 w-10 h-10 flex items-center justify-center
      rounded-full bg-white/90 backdrop-blur shadow-md text-rose-500 hover:scale-110 active:scale-85 transition-transform"
  ...
  <?php else: ?>
    <div class="relative bg-white/80 backdrop-blur rounded-3xl border border-dashed border-gray-200 p-16 text-center shadow-lg max-w-xl mx-auto">
      ...
      <p class="text-gray-500 mb-8 max-w-sm mx-auto">
        Save skills you're interested in with the heart icon and find them here.
      </p>
      <a href="search-products.php" class="inline-flex items-center gap-2 px-8 py-4 bg-gradient-to-r from-blue-600 to-indigo-600 text-white font-bold rounded-xl 
      hover:from-blue-700 hover:to-indigo-700 shadow-lg hover:shadow-xl transition-all">
        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" 
                d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" />
        </svg>
