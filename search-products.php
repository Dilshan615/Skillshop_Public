...
//fetch products
$productsQuery = "
  SELECT p.`id`, p.`title`, p.`description`, p.`image_url`, p.`price`, p.`created_at`,
         p.`level`,
...
<?php if(count($products) > 0): ?>
    <div id="products-grid" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 mb-12">
        ...
        overflow-hidden flex flex-col border border-gray-100 group">
            ...
            <div class="relative w-full h-48 bg-gray-100 overflow-hidden flex shrink-0">
              ...
              <?php if($loggedIn && $userRole == "buyer"): 
                ...
                $inw = isset($watchlistIds[$product["id"]]);
                ...
                <?php echo $inw ? "text-rose-500" : "text-gray-400 hover:text-rose-400"; ?>"
                data-product-id="<?php echo $product["id"]; ?>" data-in="<?php echo $inw ? 1 : 0; ?>"
                title="<?php echo $inw ? "Remove from watchlist" : "Add to watchlist"; ?>">
                   <svg class="w-5 h-5" fill="<?php echo $inw ? 'currentColor' : 'none'; ?>" 
    ...
    <?php if($totalPages > 1): ?>
        <nav class="flex justify-center items-center gap-1 mt-12 mb-8">
            <?php if($currentPage > 1): ?>
                <a href="?q=<?php echo urlencode($searchQuery); ?>&page=1" class="p-2 text-gray-600 hover:bg-blue-50 rounded-lg transition-colors" title="First"><<</a>
                <a href="?q=<?php echo urlencode($searchQuery); ?>&page=<?php echo $currentPage - 1; ?>" class="p-2 text-gray-600 hover:bg-blue-50 rounded-lg transition-colors" title="Previous"><</a>
            <?php endif; ?>
  ...
  if (grid) {
    ...
    try {
      ...
      if (j.success) {
        ...
        // Correct classList usage + match PHP classes
        btn.classList.remove("text-rose-500", "text-gray-400", "hover:text-rose-400");
        if (inw) {
          btn.classList.add("text-rose-500");
        } else {
          btn.classList.add("text-gray-400", "hover:text-rose-400");
        }
