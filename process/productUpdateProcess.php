...
// --- Validation rules ---
$validation = [
    ["comd" => strlen($data["title"]) >= 0 && strlen($data["title"]) <= 150, "msg" => "Title required, max 150 chars"],
    ["comd" => strlen($data["description"]) >= 0 && strlen($data["description"]) <= 1000, "msg" => "Description required, max 1000 chars"],
    ["comd" => $data["category"] > 0, "msg" => "Select valid category"],
    ["comd" => $data["price"] > 0, "msg" => "Price must be greater than 0"],
    ["comd" => in_array($data["level"],["Beginner","Intermediate","Advanced"]) , "msg" => "Invalid level"],
    ["comd" => in_array($data["status"],["active","inactive"]) , "msg" => "Invalid status"],
];
...
try{
   ...
   $result = Database::iud(
    "UPDATE `product` 
     SET `category_id`=?, `title`=?, `description`=?, `price`=?, `level`=?, `status`=?, `image_url`=? 
