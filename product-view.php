...
// Stats
$rs = Database::search(
    "SELECT AVG(`rating`) AS `r`, COUNT(*) AS `c` 
     FROM `feedback` 
     WHERE `product_id` = ?",
    "i",
    [$productId]
...
$ss = Database::search(
    "SELECT 
        COUNT(DISTINCT p.`id`) AS `tc`, 
        COUNT(DISTINCT o.`user_id`) AS `ts`, 
        AVG(COALESCE(f.`rating`, 0)) AS `tr`
     FROM `product` p
     LEFT JOIN `order` o 
...
<div class="min-h-screen" style="background:afafafc;">
    <!-- hero -->
    <div class="relative overflow-hidden bg-gradient-to-br from-slate-500 via-blue-900 to-blue-900"
        style="min-height: 280px;">
        <?php if ($p["image_url"]): ?>
            <img src="<?= $p["image_url"] ?>" class="absolute inset-0 w-full h-full object-cover opacity-10
        scale-105 blur-sm pointer-events-none">
        <?php endif; ?>
        <div class="absolute inset-0 bg-gradient-to-t from-slate-900/60 to-transparent pointer-event-none"></div>
        <div class="relative max-w-6xl mx-auto px-4 pt-10 pb-16">
            ...
            <nav class="flex items-center gap-2 text-sm text-slate-400 mb-4">
                ...
                <?php if ($p["cat_name"]): ?>
                    <span>></span>
                    <a href="search-products.php?cat=<?= $p["category_id"] ?>"
                        class="hover:text-white transition-colors"><?= $p["cat_name"] ?></a>
                <?php endif; ?>
            </nav>
            <!-- badge -->
            <div class="flex flex-wrap gap-2 mb-4">
                <span
                    class="inline-flex items-center text-xs font-bold px-3 py-1 rounded-full bg-blue-500/30 text-blue-200">
                    <?= $p["level"] ?>
                </span>
                <?php if ($p["cat_name"]): ?>
                    <span
                        class="inline-flex items-center text-xs font-semibold px-3 py-1 rounded-full bg-white/10 text-slate-300">
                        <?= $p["cat_name"] ?>
                    </span>
                <?php endif; ?>
                <?php if ($bought): ?>
                    <span
                        class="inline-flex items-center text-xs font-bold px-3 py-1 rounded-full bg-green-500/25 text-green-300">✔️
                        Enrolled</span>
                <?php endif; ?>
            </div>

            <h1 class="text-3xl md:text-4xl font-extrabold text-white leading-tight mb-4 max-a-2xl"><?= $p["title"] ?>
            </h1>
            ...
            <div class="flex flex-wrap items-center gap-5 text-sm text-slate-400">
                ...
                <span>👤 By <strong class="text-white"><?= $sellerName ?: "New" ?></strong></span>
            ...
            <div class="flex-1 min-w-0 space-y-6">
                ...
                <div class="bg-white rounded-2xl border border-slate-100 p-6 shadow-sm">
                    ...
                    <div class="grid grid-cols-2 sm:grid-cols-3 gap-3">
                        <?php foreach (["Full course access", "Lifetime access", "Direct support", "Resourses", "Certificate", "Expert content"] as $f): ?>
                ...
                <div class="bg-white rounded-2xl border border-slate-100 p-6 shadow-sm">
                    <div class="flex items-center justify-between mb-5">
                        ...
                        <div class="text-right">
                            <span class="text-3xl font-semibold text-gray-900"><?= $avgR ?: "-" ?></span>
                            <div class="text-yellow-400 text-sm" id="avg-stars"></div>
                            <span class="text-xs text-gray-600">
                                <?= $revC ?> review<?= $revC != 1 ? "s" : ""; ?>
                            </span>
                        </div>
                    </div>
                        ...
                        text-xs font-bold bg-gradient-to-br from-blue-500 to-indigo-600"><?= $init ?></div>
                                    <div class="flex-1 min-w-0">
                                        <div class="flex items-center justify-between">
                                            ...
                                            <span
                                                class="text-xs text-gray-400"><?= date("M j, Y", strtotime($rv['created_at'])) ?></span>
                                        </div>
                                        <!-- individual rating stars -->
                                        <div class="text-yellow-400 text-sm" data-star="<?= intval($rv['rating']) ?>"></div>

                                        <!-- review message -->
                                        <?php if (!empty($rv['messege'])): ?>
                                            <p class="text-sm text-gray-600 mt-1"><?= nl2br($rv['messege']) ?></p>
                                        <?php endif; ?>
                                    </div>
                                </div>
                            <?php endforeach; ?>
                    ...
                    <div class="bg-white rounded-2xl border border-slate-100 p-6 shadow-sm">
                        ...
                        <div class="grid sm:grid-cols-3 gap-4">
                            <?php foreach ($related as $r): ?>
                                <a href="product-view.php?id=<?= $r["id"] ?>" class="group block rounded-xl overflow-hidden border-slate-100 hover:border-blue-200
                            hover:shadow-md transtion-all">
                               ...
                               line-clamp-2"><?= $r["title"] ?></p>
                                        <p class="text-sm font-bold text-blue-600 mt-1">Rs. <?= number_format($r["price"], 2) ?>
                                        </p>
                                    </div>
                                </a>
                            <?php endforeach; ?>
