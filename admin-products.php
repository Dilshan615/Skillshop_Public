...
<body class="bg-slate-50 min-h-screen">
            ...
            <header
                ...
                <div class="flex items-center gap-4">
                    ...
                    <input type="text" id="productSearch" onkeyup="filterProducts();" placeholder="Search products...."
                        class="px-4 py-2 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:ring-4 focus:ring-4 focus:ring-blue-500/10 outline-none" />
                </div>
            </header>
            ...
            <div class="p-8">
                <div class="grid grid-cols-1 gap-6" id="productsGrid">
                    <?php while ($prod = $products->fetch_assoc()): ?>
                        <div
                            class="product-item bg-white p-6 rounded-2xl shadow-sm border border-slate-100 flex flex-col lg:flex-row gap-6 items-center transition-all hover:shadow-md">
                            <img src="<?= $prod["image_url"] ?>"
                                class="w-24 h-24 rounded-xl object-cover bg-slate-100 flex-shrink-0 cursor-pointer"
                                onclick="viewProduct(<?= $prod['id']; ?>);" />
                            <div class="flex-1">
                                <div class="flex items-center gap-3 mb-1">
                                    <span
                                        class="px-2 py-0.5 bg-blue-50 text-blue-600 text-[9px] font-black uppercase rounded-md"><?= $prod["category_name"] ?></span>
                                    <span id="status-<?= $prod["id"]; ?>"
                                        class="text-[10px] font-bold uppercase <?= ($prod["status"] == "active") ? "text-green-500" : "text-red-500"; ?>"><?= $prod["status"]; ?></span>
                                </div>
                                <h3 class="text-lg font-bold text-slate-900 prod-title cursor-pointer"
                                    onclick="viewProduct(<?= $prod['id']; ?>);"><?= $prod["title"]; ?></h3>
                                <p class="text-xs text-slate-900 prod-seller">By
                                    <?= $prod["fname"] . " " . $prod["lname"]; ?></p>
                            </div>
                            <div class="text-center lg:text-right flex-shrink-0">
                                ...
                                <p class="text-xl font-black text-slate-900">Rs. <?= number_format($prod["price"]); ?></p>
                            </div>
                            <div class="flex items-center gap-3 flex-shrink-0">
                                <button onclick="viewProduct(<?= $prod['id']; ?>);"
                                    class="px-6 py-2.5 rounded-xl text-xs font-bold bg-slate-100 hover:bg-slate-200 transition-all">View
                                    Details</button>
                                <button onclick="toggleProductStatus(<?= $prod['id']; ?>);" id="btn-<?= $prod['id']; ?>"
                                    class="px-6 py-2.5 rounded-xl text-xs font-bold transition-all <?= ($prod["status"] == "active") ? "bg-red-50 text-red-500 hover:bg-red-600 hover:text-white" : "bg-green-500 text-white hover:bg-green-700"; ?>">
    ...
    <div id="productModal" class="hidden fixed inset-0 z-50 overflow-y-auto">
        <div class="flex items-center justify-center min-h-screen p-4">
            ...
            <div
                class="bg-white rounded-3xl shadow-2xl overflow-hidden w-full max-w-2xl relative z-10 animate-in fade-in zoom-in duration-300">
                <button onclick="closeProductModal();"
                    class="absolute top-6 right-6 text-slate-400 hover:text-slate-600 transition-colors text-2xl font-bold">X</button>
                ...
                <div class="p-8">
                    <div class="flex justify-between items-center mb-6">
                        ...
                        <button onclick="closeCategoryModal();"
                            class="text-slate-400 hover:text-slate-600 transition-colors text-2xl font-bold">X</button>
                    </div>

                    <!-- Category Registration Form -->
                    <div class="bg-slate-50 p-6 rounded-2xl mb-8">
                        <h4 class="text-xs font-bold text-slate-400 uppercase tracking-widest mb-4">Add New Category
                        </h4>
                        <div class="flex gap-2">
                            <input type="text" id="catName" placeholder="e.g. Designing"
                                class="flex-1 px-4 py-2.5 bg-white border border-slate-200 rounded-xl text-sm focus:ring-blue-500/10 outline-none" />
                            <button onclick="registerCategory();"
                                class="bg-blue-600 text-white px-4 py-2.5 rounded-xl text-sm font-bold hover:bg-blue-700 transition-all shadow-lg active:scale-95">Add</button>
                        </div>
                        <p id="catMsg" class="hidden mt-1 text-xs font-medium text-center"></p>
                    </div>

                    <!-- Categories List -->
                    <h4 class="text-xs font-bold text-slate-600 uppercase tracking-widest mb-4">Existing Categories</h4>
                    <div id="categoryList" class="space-y-2 max-h-60 overflow-y-auto pr-2 custom-scrollbar"></div>
                </div>
            </div>
        </div>
    </div>
                ...
                if (data.success) {
                    ...
                    content.innerHTML = `
                      <div class="flex flex-col md:flex-row gap-8">
                       ...
                       <div class="flex-1">
                       <span class="px-2 py-0.5 bg-blue-50 text-slate-900 leading-tight mb-2 inline-block">${p.category_name}</span>
